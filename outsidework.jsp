...
function initControl() {
	...
	$("#customerSeq").select2({minimumResultsForSearch: Infinity});
	...
	, processing:true
	, ajax: {
        url: '/admin/getOutsideworkListAjax.do', //url 정보 수정
        type: 'POST'
    }
   , order: [[ 0, 'desc' ]]
    , columns: [
       ...
       { 'data': 'customerName' ,createdCell:function (td, cellData, rowData, row, col){
    	 //td cursor 스타일 변경
  			$(td).css('cursor', 'pointer');			
	            $(td).click(function(e){
	            	...
	            	//클릭한 직원의 상세정보 불러 오기
	              	postAjax('/admin/selectOutsideworkAjax.do', {seq:rowData.seq}, function(data, status){
        ...
        }, className:'text-center'},
        {
            ...
            {
         	   var datehtml = "";
            	datehtml+=rowObject.startDate;
            	datehtml+='~';
            	datehtml+=rowObject.endDate;
        ...
        },
        {
            ...
            {
         	   var datehtml = "";
            	datehtml+=rowObject.startTime;
            	datehtml+='~';
            	datehtml+=rowObject.endTime;
...
function initEvent() {
   ...
   $(document).on('click', 'a[role=dataEdit]', function(){
       ...
       //안내 문구 변경
         swalInit.fire({
             ...
             if(result.value) {
               //예
               //삭제 Url 변경
                   postAjax('/admin/deleteOutsideworkAjax.do', {seq:seq}, function(data, status){
    ...
    $('#modalSave').on('show.bs.modal', function(e) {
       ...
       {
            postAjax('/admin/selectOutsideworkAjax.do', {seq:$(this).data('seq')}, function(data, status){
               console.log(data);
               ...
               $("#customerSeq").val(data.data.customerSeq).trigger("change.select2");
            });
        }
    });
   ...
   //저장 이벤트
    $('#btnDataSave').click(function(){
    	if ($('#form').valid())
    	{
               ...
               ajax(null, '/admin/mergeOutsideworkAjax.do', formData, function(data, status){
                   showAjaxMessage(data);
                   if (data.isSuccess === '1')
                   {
                      //목록 새로고침
...
<div class='card'>
	<div class='card-header header-elements-inline'>
		<h5 class='card-title font-weight-bold'>
			<i class='icon-chevron-right mr-1'></i>외근 관리
		</h5>
					...
					<div class='datatable-scroll'>
						<table class='detailtable mb-3'>
							...
							<tbody>
								...
								<tr>
									<th>고객사</th>
									<td><select id="customerSeq" name='customerSeq'
										class="from-control">
											<option selected value="" hidden="">고객사 선택</option>
											<c:forEach items="${getCustomerList}" var="getCustomerList">
												<option value="${getCustomerList.seq}"><c:out
														value="${getCustomerList.customerName}" /></option>
											</c:forEach>
									</select></td>
								</tr>
								<tr>
                                <th>외근 시작일 ~ 종료일</th>
                                <td>시작일<input id='startDate' name='startDate' maxlength='20' class='form-control' type='text' placeholder='외근 시작일'>종료일 
                                <input id='endDate' name='endDate' maxlength='20' class='form-control' type='text' placeholder='외근 종료일'></td>                             
                           		</tr>
								<tr>
									<th>방문시간</th>
									<td>시작시간<input id='startTime' name='startTime' maxlength='20' class='form-control' type='text' placeholder='시작시간'>
									종료시간 <input id='endTime' name='endTime' maxlength='20' class='form-control' type='text' placeholder='종료시간'></td>
								</tr>
								
								<tr>
									<th>작업내용</th>
									<td><input id='textArea' name='textArea' maxlength='20'
										class='form-control' type='text' placeholder='작업내용'></td>
								</tr>
                ...
                <div class='datatable-scroll'>
                    ...
                    <table class='detailtable mb-3'>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th>시작일 ~ 종료일</th>
                                <td><label id='lblstartDate'></label> ~ <label id='lblendDate'></label></td>
                            </tr>
                            <tr>
                                <th>시작시간 ~ 종료시간</th>
                                <td><label id='lblstartTime'></label> <label id='lblendTime'></label></td>
                            </tr>
                            <tr>
                                <th>작업내용</th>
                                <td><label id='lbltextArea'></label></td>
                            </tr>
